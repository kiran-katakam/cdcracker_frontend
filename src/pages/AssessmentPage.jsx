  ...
  import { Search, Code2, HelpCircle, Copy, CheckCircle, Plus, Pencil, Trash2, ChevronDown } from 'lucide-react';
  ...
  const [expandedCode, setExpandedCode] = useState(new Set()); // collapsed by default

  const toggleCode = (id) => setExpandedCode((prev) => {
            ...
            {filteredCoding.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {filteredCoding.map((q, i) => {
                  const expanded = expandedCode.has(q._id);
                  return (
                    <div key={q._id} className="code-block">
                      {/* Clickable header */}
                      <div
                        className="code-block-header"
                        onClick={() => toggleCode(q._id)}
